...
//		return con;
	...
	public int getCommentNum() 
	{
		...
		try {
			...
			sql.append("select max(num) as max from comment");
	...
	public List getCommentList(int startRow, int pageSize,int board ) {
		...
		try {
			...
			String sql = "select c.* from board b join comment c on b.num = c.board where b.num=? order by num desc limit ?,?";
	...
	public int getCommentCount(int num) {
		...
		try {
			con = getConnection();
			String sql = "select count(*) as count from comment where board=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
