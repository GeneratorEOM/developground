...
//		return con;
	...
	public int getCommentNum() 
	{
		...
		try {
			...
			sql.append("select max(num) as max from pcomment");
	...
	public void insertComment(PhotoCommentBean cb) {
	...
	public List getCommentList(int startRow, int pageSize,int board ) {
		...
		try {
			...
			con = getConnection();
			String sql = "select c.* from pboard b join pcomment c on b.num = c.board where b.num=? order by num desc limit ?,?";
			...
			PhotoCommentBean cd = new PhotoCommentBean();
	...
	public int getCommentCount() {
		...
		try {
			con = getConnection();
			String sql = "select count(*) as count from pcomment";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public int getCommentCount(int num) {
		...
		try {
			con = getConnection();
			String sql = "select count(*) as count from pcomment where board=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
	...
	public void updateComment(String content, int num) {
		...
		try {
			...
			con = getConnection();
			String sql = "update pcomment set content=? where num=?";
			pstmt = con.prepareStatement(sql);
